...
func _ready():
	...
	out_of_range_timer.connect("timeout",Callable(self,"_on_OutOfRangeTimer_timeout"))
...
func update(_delta):
	if enemy_wr and enemy_wr.get_ref():
		if has_reached_enemy and enemy.position.distance_to(owner.position) > 1500:
		...
		"""
		var t = Timer.new()
		t.set_wait_time(randf_range(2, 4))
		self.add_child(t)
		t.start()
		t.connect("timeout",Callable(self,"enter"))
	...
	if enemy_wr and enemy_wr.get_ref():
		if has_reached_enemy:
			if get_away_from_the_enemy:
				...
				if owner.position.distance_to(owner.input.target) < 100: # distancia a la attack pos
					get_away_from_the_enemy = false
			else: # disa
				if not owner.shooting.enemy_in_range and out_of_range_timer.is_stopped():
					out_of_range_timer.wait_time = 10#randf_range(5, 14)
					out_of_range_timer.start()
					#no cal - elif owner.get_node("Shooting").enemy_in_range:
					#	timer.stop()
					# girar de pressa
				owner.input.des_throttle = 0.4
				owner.input.target = enemy.position
		else:
			owner.input.des_throttle = 1.0
			owner.input.target = enemy.position + attack_rel_pos # q si no sumes la relativa es xoquen
			if owner.position.distance_to(enemy.position) < 700:#400: # distancia a l'enemic
				has_reached_enemy = true
				total_attack_pos = enemy.position + attack_rel_pos
...
func change_rel_pos():
	attack_rel_pos = Vector3(randf_range(-200, 200), randf_range(-200, 200), randf_range(-200, 200))
